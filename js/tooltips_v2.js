...
$('#validationFunction').click(function() {
	...
	if (allScreensAmount < 1) {
		$.dialog({
			...
			icon: 'fa fa-warning',
			title: '<p class="alert-title">Error</p>',
			content: '<p class="alert-text">You must choose at least 1 screen. You currently have ' + allScreensAmount + '.</p>',
			opacity: 0.6,
			backgroundDismiss: true,
			closeIcon: true,
			closeIconClass: 'fa fa-times',
	...
	if ($('#useraccept').prop('checked') === false ) {
        ...
        $( "#validationFunction" ).hide();
		$( "#validationFunction2" ).hide();
